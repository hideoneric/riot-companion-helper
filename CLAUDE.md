...
src/preload/    → Context bridge (index.ts = main window)
...
index.ts  →  Poller (tick every N seconds)
              ...
              ├─ launches/kills BlitzLauncher / ValorantTrackerLauncher
...
- **`poller.ts`**: Polls on a `setInterval`. Tracks two game pairs: League+Blitz and Valorant+ValorantTracker. `broadcastIfChanged()` prevents redundant IPC sends.
- **`launcher.ts`**: `BlitzLauncher` (spawn/taskkill by PID+name). `ValorantTrackerLauncher` (spawn Overwolf with `-launchapp <id>`; kill by PowerShell window title match).
...
- **`detector.ts`**: Auto-detect `Blitz.exe` and `Overwolf.exe` paths on disk.
- **`settings-store.ts`**: `electron-store` v8 (v11+ is ESM-only, breaks CJS main process). Fields: `blitzPath`, `valorantTrackerPath`, `launchWithWindows`, `pollingInterval`, `monitoringEnabled`.
...
| `detectOverwolf()` | renderer→main | auto-detect Overwolf.exe |
...
- **`HomePage`**: monitoring status badge + two grouped process cards (League ecosystem, Valorant ecosystem) + scrollable activity log
- **`SettingsPage`**: General (Blitz path, Valorant Tracker path, polling interval) and Behavior (monitoring toggle, launch with Windows)
...
- **Poller startup condition**: Poller only starts if `monitoringEnabled && (blitzPath || valorantTrackerPath)`. If neither path is set, the poller stays idle.
